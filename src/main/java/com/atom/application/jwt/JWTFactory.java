package com.atom.application.jwt;
...
import com.atom.application.properties.JWTConfigurationProperties;
...
public class JWTFactory {
    ...
    public class JWT {
        ...
        private JWT(String username, Collection<? extends GrantedAuthority> authorities) {
            ...
            this.stringJwt = Jwts.builder().setClaims(authoritiesClaims).setSubject(username).setIssuedAt(issuedAt)
                    .setExpiration(expiration).signWith(SignatureAlgorithm.HS512, configProps.getSecret()).compact();
        }

        private JWT(String jwtString) {
            Claims claims = Jwts.parser().setSigningKey(configProps.getSecret()).parseClaimsJws(jwtString).getBody();
